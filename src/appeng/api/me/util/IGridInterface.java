...
/**
 * Lets you access network related features. You will mostly care about
 * "getCellArray()" which returns the IMEInventory for the entire network...
 */
public interface IGridInterface
{
	...
	/**
	 * returns a single IMEInventory that represents the entire network, and all
	 * crafting available.
	 * 
	 * @return
	 */
	...
	/**
	 * creates a crafting job to push the item out of IPushable, and to enable
	 * crafting to acocomplish this goal.
	 * 
	 * @param willAdd
	 * @param out
	 * @param allowCrafting
	...
	/**
	 * get a numeric id for this grid, used internally for cable animations, and
	 * not much else.
	 * 
	 * @return
	 */
	...
	/**
	 * Inform the netowrk that these items were removed from storage ( don't do
	 * this for items remove though the grid, it already knows about those. )
	 * 
	 * @param removed
	 */
	...
	/**
	 * Inform the network that these items were added to storage ( don't do this
	 * for items added via the grid, it already knows about those. )
	 * 
	 * @param vo
	 */
	...
	/**
	 * return a cache by its identifier ( the number you get when you register
	 * your cache with AE )
	 * 
	 * @param id
	 * @return
	...
	/**
	 * inform the network that energy was transfered between two nodes on the
	 * network, this will be used to calculates future information, but also
	 * causes the network to flash presently.
	 * 
	 * this will automatically be handled by the standard extractItem / addItem
	 * API in the future, only Processes that skip the vanilla AE storage system
	 * should use this.
	 * 
	 * @param a
	 * @param b
	...
	/**
	 * posts an event into the network, blocks with the event listeners on them
	 * will receive them.
	 * 
	 * @param ev
	 * @return
	...
	/**
	 * Returns if the system can provide this much power, returns the amount of
